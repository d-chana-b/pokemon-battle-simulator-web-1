...
import com.pokemonsimulator.demo.service.PokemonMoveListService;
...
public class PokemonController {
    ...
    @Autowired
    PokemonMoveListService pokeMoveService;
    ...
    public String viewPokemonDetails(Model model, @PathVariable Long id) {
        Pokemon pokemon = pokemonService.getPokemonById(id).get();
        model.addAttribute("pokemon", pokemon);
        model.addAttribute("moves", moveService.getAllMoves());
        model.addAttribute("pokemonmoves", pokeMoveService.getAllMovesByPokemonId(pokemon));

        // List<PokemonMoveList> test = pokeMoveService.getAllMovesByPokemonId(pokemon);
        // for (PokemonMoveList pokemonMoveList : test) {
        // pokemonMoveList.getMove().getMoveName();
        // }
    ...
    public String addMoveToPokemon(@RequestParam("pokemonId") Long pokemonid, @RequestParam("moveId") Long moveid,
        ...
        // List<PokemonMoveList> allMovesByPokemon =
        // pokeMoveService.findByAllMovesByPokemonId(pokemon.get().getPokemonName());
        // PokemonMoveList temp =pokeMoveEntity;
        // for (PokemonMoveList pokemonMoveEntity : allMovesByPokemon){
        // if(pokemonMoveEntity.getMove().equals(move.get())){
        // temp = pokemonMoveEntity;
        // pokeMoveService.saveMoveToPokemon(pokemonMoveEntity);
        // }
        // }

        // wala ka pa ginagawa pang check pag naduplicate ung entry hahahah tska mo na
        // pagisipan yan
        pokeMoveService.saveMoveToPokemon(pokemon.get(), move.get(), moveCategory);
